...
import {
  ...
  Monitor, ExternalLink, Code2, Tag,
  Type, Palette, Navigation,
  Stethoscope, Wine, Sparkles, Dumbbell, UtensilsCrossed, Briefcase,
  Signpost,
} from 'lucide-react'
...
const GALERIA = [
  { slug: 'letras-acrilico-fachada', label: 'El letrero que para a la gente',   sub: 'Letras volumétricas · Acrílico · Tu logo en 3D', glow: '#C4953A', Icon: Type,       span: 'big'  },
  { slug: 'letrero-led-tienda',       label: 'Vende también de noche',            sub: 'Letreros LED · Backlit · Visible las 24h',       glow: '#D4B840', Icon: Lightbulb,  span: ''     },
  { slug: 'vinilo-vitrina',           label: 'Transforma cualquier superficie',   sub: 'Vinilos · Ploteo · Corte láser',                  glow: '#7A6B9B', Icon: Palette,    span: ''     },
  { slug: 'totem-publicitario',       label: 'Te ubican desde la calle',          sub: 'Tótem · PVC · Foam · Señalización exterior',      glow: '#4A8B6A', Icon: Signpost,   span: 'wide' },
  { slug: 'letras-mdf-restaurante',  label: 'Transmite calidad en cada detalle', sub: 'MDF · Madera · Doradas · Barnizadas',             glow: '#C4953A', Icon: Layers,     span: ''     },
  { slug: 'senaletica-oficina',       label: 'Guía a tus clientes adentro',       sub: 'Señalética · Directorio · Rotulación interna',    glow: '#4A7A9B', Icon: Navigation, span: ''     },
]
...
const PRODUCTOS = [
  ...
  },
  {
    ...
    glow: '#C4953A', tag: 'Efecto inmediato',
  },
  {
    icon: Monitor,
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      for (const p of pts) {
        ...
        ctx.fillStyle = 'rgba(196,149,58,.75)'
        ctx.fill()
      }
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (d < 130) {
            ...
            ctx.strokeStyle = `rgba(196,149,58,${.18 * (1 - d / 130)})`
            ctx.lineWidth = .6
            ctx.stroke()
          }
        }
      ...
      for (const p of pts) {
        ...
        if (d < 160) {
          ...
          ctx.strokeStyle = `rgba(196,149,58,${.45 * (1 - d / 160)})`
          ctx.lineWidth = .9
          ctx.stroke()
        }
      }
...
const CSS = `
  ...
  .lst-logo:hover { filter:grayscale(0%) opacity(1); border-color:#C4953A; }
  ...
  .step-wrap:hover .step-n {
    color:#C4953A;
    -webkit-text-stroke-color:#C4953A;
  }
  ...
  return (
    <div style={{ fontFamily: "'Poppins', system-ui, sans-serif", background: '#000', color: '#fff', overflowX: 'hidden' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(0,0,0,.88)', backdropFilter: 'blur(14px)', borderBottom: '1px solid #1A1A1A' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ background: '#C4953A', borderRadius: 8, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 14px #C4953A80' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, color: '#555', display: 'flex', alignItems: 'center', gap: 4 }}>
              <MapPin size={11} color="#C4953A" /> {city.name}
            </span>
            <a href={WA_FAST} target="_blank" rel="noopener noreferrer"
              onClick={() => { px('Contact'); ga('contact', { method: 'whatsapp' }) }}
              style={{ background: '#C4953A', color: '#000', fontWeight: 800, fontSize: 12, padding: '9px 18px', borderRadius: 30, display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none', boxShadow: '0 0 18px #C4953A60' }}>
      ...
      <section style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 700, height: 700, borderRadius: '50%', background: 'radial-gradient(circle, rgba(196,149,58,.09) 0%, transparent 70%)', pointerEvents: 'none' }} />

        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '0 24px', maxWidth: 860, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, border: '1px solid #C4953A50', borderRadius: 30, padding: '6px 16px', marginBottom: 28, background: 'rgba(255,107,0,.07)' }}>
            <MapPin size={11} color="#C4953A" />
            <span style={{ fontSize: 11, fontWeight: 700, color: '#C4953A', letterSpacing: 1.5 }}>LETREROS · {city.name.toUpperCase()} · REGIÓN DEL MAULE</span>
          </div>
          ...
          <p style={{ fontSize: 12, color: '#C4953A', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 44 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', marginBottom: 60 }}>
            <a href={WA} target="_blank" rel="noopener noreferrer"
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#C4953A', color: '#000', fontWeight: 800, fontSize: 14, padding: '14px 30px', borderRadius: 40, textDecoration: 'none', boxShadow: '0 0 32px #C4953A70, 0 8px 24px rgba(0,0,0,.5)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 32, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#444' }}>
                <Icon size={12} color="#C4953A" /> {text}
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ background: '#000', borderTop: '1px solid #1A1A1A', borderBottom: '1px solid #1A1A1A', padding: '32px 24px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20, textAlign: 'center' }} className="lst-stats">
          {[
            ...
            <div key={s.l}>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 30, fontWeight: 700, color: '#C4953A', textShadow: '0 0 18px #C4953A60', marginBottom: 4 }}>{s.v}</div>
      ...
      <section style={{ background: '#030303', padding: '88px 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#C4953A', marginBottom: 14 }}>Trabajos realizados</p>
              ...
              <TiltCard key={item.slug} glow={item.glow} style={item.span === 'big' ? { gridColumn: 'span 2', gridRow: 'span 2' } : item.span === 'wide' ? { gridColumn: 'span 2' } : {}}>
                <div style={{ width: '100%', height: '100%', minHeight: 240, position: 'relative', background: '#0A0A0A' }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    <div style={{ filter: `drop-shadow(0 0 20px ${item.glow})` }}>
                      <item.Icon size={item.span === 'big' ? 64 : 40} color={item.glow} strokeWidth={1.25} />
                    </div>
                  </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 40 }}>
            ...
            <a href={WA} target="_blank" rel="noopener noreferrer"
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: 'transparent', color: '#C4953A', fontWeight: 800, fontSize: 14, padding: '12px 24px', borderRadius: 30, textDecoration: 'none', border: '1px solid #C4953A60', boxShadow: '0 0 20px #C4953A30' }}>
      ...
      <section style={{ background: '#060606', padding: '96px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#C4953A', marginBottom: 14 }}>Nos adaptamos a tu rubro</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="sec-grid">
            {SECTORES.map(s => (
              ...
              >
                ...
                <div style={{ marginBottom: 16, filter: `drop-shadow(0 0 12px ${s.color}60)` }}>
                  <s.Icon size={30} color={s.color} strokeWidth={1.5} />
                </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 44 }}>
            ...
            <a href={WA} target="_blank" rel="noopener noreferrer"
              onClick={() => { px('Contact'); ga('contact', { method: 'whatsapp' }) }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'transparent', color: '#C4953A', fontWeight: 700, fontSize: 13, padding: '11px 22px', borderRadius: 30, textDecoration: 'none', border: '1px solid #C4953A50' }}>
      ...
      <section style={{ background: '#000', padding: '88px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#C4953A', marginBottom: 14 }}>Qué fabricamos</p>
      ...
      <section style={{ background: '#030303', padding: '88px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#C4953A', marginBottom: 14 }}>Sin riesgos, sin sorpresas</p>
      ...
      <section style={{ background: '#000', padding: '88px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }} className="lst-grid">
          <div>
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#C4953A', marginBottom: 16 }}>Llegamos donde estés</p>
            ...
            {['Provincia de Talca (10 comunas)','Provincia de Curicó (9 comunas)','Provincia de Linares (8 comunas)','Provincia de Cauquenes (3 comunas)'].map(i => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#C4953A', boxShadow: '0 0 8px #C4953A', flexShrink: 0 }} />
                <span style={{ fontSize: 13, color: '#555' }}>{i}</span>
              </div>
            ))}
          </div>
          <div style={{ background: '#0A0A0A', border: '1px solid #1A1A1A', borderRadius: 20, padding: '36px 32px', boxShadow: '0 0 60px rgba(196,149,58,.06)' }}>
            ...
            {[
              ...
              <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <CheckCircle2 size={13} color="#C4953A" style={{ flexShrink: 0 }} />
                <span style={{ fontSize: 13, color: '#555' }}>{item}</span>
              </div>
            ))}
            <a href={WA} target="_blank" rel="noopener noreferrer"
              onClick={() => { px('Contact'); ga('contact', { method: 'whatsapp' }) }}
              style={{ marginTop: 28, display: 'inline-flex', alignItems: 'center', gap: 8, background: '#C4953A', color: '#000', fontWeight: 800, fontSize: 13, padding: '12px 22px', borderRadius: 30, textDecoration: 'none', boxShadow: '0 0 22px #C4953A60' }}>
      ...
      <section style={{ background: '#030303', padding: '72px 0', overflow: 'hidden', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center', padding: '0 24px', marginBottom: 40 }}>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#C4953A', marginBottom: 12 }}>Nos eligen en todo Chile</p>
      ...
      <section style={{ background: '#000', padding: '100px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden', borderTop: '1px solid #111' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 600, height: 400, background: 'radial-gradient(ellipse, rgba(196,149,58,.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', maxWidth: 560, margin: '0 auto' }}>
          <div style={{ marginBottom: 20, filter: 'drop-shadow(0 0 24px #C4953A60)' }}>
            <Signpost size={52} color="#C4953A" strokeWidth={1.25} />
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', marginBottom: 20 }}>
            <a href={WA} target="_blank" rel="noopener noreferrer"
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#C4953A', color: '#000', fontWeight: 800, fontSize: 15, padding: '16px 32px', borderRadius: 40, textDecoration: 'none', boxShadow: '0 0 40px #C4953A70, 0 8px 24px rgba(0,0,0,.5)' }}>
      ...
      <footer style={{ background: '#000', padding: '20px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            <div style={{ background: '#C4953A', borderRadius: 6, width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
